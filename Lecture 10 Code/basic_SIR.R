...
disease.model <- function (t, x, params) {
  S <- x[1]
  I <- x[2]
  R <- x[3]
  with (
...
out <- as.data.frame(lsoda(xstart,times,disease.model,params))
...
legend("topleft", c("Suseptible","Infected","Removed"), col=c("darkgreen","red3","blue"), lwd=c(2,2,2), bty='y', ncol=3)